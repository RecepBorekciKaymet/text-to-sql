...
def execute_and_report_helper_2(message) -> dict:
    """
    Processes a user's natural language query by:
      1. Converting the NLQ to an SQL query using convert_nlp_to_sql.
      2. Executing the SQL query using execute_sql_query.
      3. Creating a custom system prompt and user message for GPT-4o to generate a final report.
      4. Returning a dictionary with the SQL query, the query results, and the final report.

    Args:
        message (str): The natural language query provided by the user.

    Returns:
        dict: {
            "sql_query": <generated SQL query>,
            "results": <structured SQL results>,
            "final_report": <final report generated by Nova>
        }
    """
    ...
    # Step 3: Create new system and user messages for the final GPT-4o call.
    # (Update the examples below as needed.)
    system_prompt = (
    ...
    user_prompt = (
        "Here's the NLQ written by the user: " + message + "\n"
        "Here's the SQL query corresponding to this NLQ generated by another LLM: " + sql_query + "\n"
        "And here are the results returned by executing this sql_query: " + json.dumps(sql_results)
    )
    messages = [
    ...
    # Step 4: Call GPT-4o to generate the final report
    response = openai.chat.completions.create(
        model=MODEL,
        messages=messages
    )
    ...
    return {"sql_query": sql_query, "results": sql_results, "final_report": final_report}
